...
class DummyDataset(Dataset):
    ...
    def glob_frames(self, scenes):
        for scene in scenes:
            ...
            glob_path = osp.join(self.root_dir, 'data_odometry_color', 'dataset', 'sequences', scene, 'image_2', '*.png')
            cam_paths = sorted(glob.glob(glob_path))
            # load calibration
            # calib = self.read_calib(osp.join(self.root_dir, 'dataset', 'sequences', scene, 'calib.txt'))
            calib = self.read_calib(osp.join(self.root_dir, 'data_odometry_calib', 'dataset', 'sequences', scene, 'calib.txt'))
            ...
            for cam_path in cam_paths:
                ...
                data = {
                    ...
                    #                        frame_id + '.bin'),
                    'lidar_path': osp.join(self.root_dir, 'data_odometry_velodyne', 'dataset', 'sequences', scene, 'velodyne',
                                           frame_id + '.bin'),
                    # 'label_path': osp.join(self.root_dir, 'dataset', 'sequences', scene, 'labels',
                    #                        frame_id + '.label'),
                    'label_path': osp.join(self.root_dir, 'data_odometry_labels', 'dataset', 'sequences', scene, 'labels',
                                           frame_id + '.label'),
                    'proj_matrix': proj_matrix,
                    'proj_matrix_Tr': calib['Tr']
                }
    ...
    def __getitem__(self, index):
        ...
        pts_cam_coord = (data_dict['proj_matrix_Tr'] @ points_hcoords.T).T
        ...
        data_dict['pts_cam_coord'] = pts_cam_coord[keep_idx_img_pts][:, :3]
        data_dict['intensity'] = intensity[keep_idx]
...
def preprocess(split_name, root_dir, out_dir):
    ...
    for i, data_dict in enumerate(dataloader):
        ...
        out_dict = {
            ...
            'intensity': data_dict['intensity'].numpy(),
...
if __name__ == '__main__':
    ...
    root_dir = '/media/wy/dataset/kitti'
    out_dir = '/media/wy/dataset/kitti/data_odometry_intensity_preprocess'
