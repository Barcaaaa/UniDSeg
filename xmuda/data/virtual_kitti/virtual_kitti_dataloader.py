...
class VirtualKITTISCN(VirtualKITTIBase):
    ...
    def __getitem__(self, index):
        ...
        # compute grayscale as virtual intensity
        # intensity = (0.299 * data_dict['RGBs'][:, 0] + 0.587 * data_dict['RGBs'][:, 1] + 0.114 * data_dict['RGBs'][:, 2])
        # intensity = intensity.numpy().reshape(-1, 1)
        ...
        seg_label[seg_label == 99] = len(self.label_mapping) - 1  # index=99, out of index
        ...
        if self.crop_size:
            ...
            if valid_crop:
                ...
                points_cam_coords = points_cam_coords[keep_idx]
                points_img[:, 0] -= top
                points_img[:, 1] -= left
        ...
        # compute sparse depth map and intensity map
        depth = np.zeros((image.size[1], image.size[0]))
        depth[img_indices[:, 0], img_indices[:, 1]] = points_cam_coords[:, 2] / 100.  # normalize, max_depth=99.9
        # intensity_map = np.zeros((image.size[1], image.size[0]))
        # intensity_map[img_indices[:, 0], img_indices[:, 1]] = intensity[:, 0]
        ...
        if self.use_color:
            out_dict['feats'] = out_dict['img'][:, out_dict['img_indices'][:, 0], out_dict['img_indices'][:, 1]].T  # use rgb feature
        else:
