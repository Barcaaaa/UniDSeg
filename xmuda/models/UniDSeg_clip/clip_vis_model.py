...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, input_resolution=224, patch_size=32, width=768, layers=12, heads=12, output_dim=512,
                 drop_path_rate=0.0, out_indices=[3, 5, 7, 11], pretrained=None, get_embeddings=False, num_channel=3):
        super().__init__()
            ...
            if 'positional_embedding' in state_dict.keys():
                if self.positional_embedding.shape != state_dict['positional_embedding'].shape:
                    ...
                    spatial_pos = F.interpolate(
                        state_dict["positional_embedding"][1:, ].reshape(1, leng, leng, self.width).permute(0, 3, 1, 2),
    ...
    def forward(self, x: torch.Tensor, dep: torch.Tensor):
        inp = x
        ...
        for i, blk in enumerate(self.transformer.resblocks):
            x[1:, :, :] = x[1:, :, :] + prompt[i].permute(1, 0, 2)

            x = blk(x)

            # 确认是否需要batch为第一个维度
            x = self.tunability.forward(x, layer=i, batch_first=False, has_cls_token=True)
